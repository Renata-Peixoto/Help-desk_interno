...
function saveTicketResponse(PDO $db, int $ticketId, string $sender, string $message, ?string $adminEmail = null): bool
{
    ...
    $existing = $db->prepare('SELECT id FROM ticket_responses WHERE ticket_id = ? AND sender = ? AND message = ? AND admin_email <=> ? LIMIT 1');
    $existing->execute([$ticketId, $sender, $message, $adminEmail]);
    if ($existing->fetch()) {
        return true;
    }

    $stmt = $db->prepare('INSERT INTO ticket_responses (ticket_id, sender, message, admin_email) VALUES (?, ?, ?, ?)');
