...
<<<<<<< HEAD
    if (isset($_POST['user_reply_ticket'])) {
        ...
        if (!$ticket || ($ticket['user_id'] !== (int)$currentUser['id'] && strtolower($ticket['email']) !== strtolower($currentUser['email']))) {
            $message = 'Você não tem permissão para responder este chamado.';
        } elseif ($ticket['status'] === 'Resolvido') {
            $message = 'Este chamado já está resolvido e não aceita novas respostas.';
        } elseif ($replyMessage === '') {
            $message = 'Digite uma mensagem para responder ao chamado.';
        } else {
            $saved = saveTicketResponse($db, $ticketId, 'user', $replyMessage, $currentUser['email']);
            if ($saved) {
                $body = '<p>Olá equipe,</p>' .
                    '<p>Uma nova resposta foi enviada pelo usuário no ticket #' . $ticketId . '.</p>' .
                    '<p>' . nl2br(htmlspecialchars($replyMessage, ENT_QUOTES, 'UTF-8')) . '</p>';
                sendEmailSMTP(MAIL_ADMIN_ADDRESS, 'Nova resposta do usuário no ticket #' . $ticketId, $body);
                $message = 'Resposta enviada com sucesso.';
            } else {
                $message = 'Não foi possível enviar a resposta neste momento.';
            }

            header('Location: index.php?status=reply-success');
            exit;
        }
    } else {
...
<<<<<<< HEAD
                                ...
                                <div class="mb-4 mt-4">
                                    <strong class="text-arruda-deep d-block mb-2"><i class="fas fa-comments me-1"></i> Histórico do Chamado</strong>
                                    <?php if (count($responses) === 0): ?>
                                        <p class="text-muted" style="font-size: 0.95rem;">O suporte ainda não respondeu este chamado.</p>
                                    <?php else: ?>
                                        <div class="d-flex flex-column gap-2">
                                            <?php foreach ($responses as $res): ?>
                                                <div class="p-3 rounded-2" style="background: <?= $res['sender'] === 'admin' ? '#e3f2fd' : '#f8f9fa' ?>; border: 1px solid <?= $res['sender'] === 'admin' ? '#bbdefb' : '#e9ecef' ?>;">
                                                    <small class="text-muted d-block mb-1 font-weight-bold">
                                                        <i class="fas <?= $res['sender'] === 'admin' ? 'fa-headset' : 'fa-user' ?> me-1"></i>
                                                        <?= $res['sender'] === 'admin' ? 'Suporte Arruda' : 'Você' ?> — <?= htmlspecialchars(date('d/m/Y H:i', strtotime($res['created_at']))) ?>
                                                    </small>
                                                    <div style="color: var(--arruda-text); line-height: 1.5; white-space: pre-line;"><?= nl2br(htmlspecialchars($res['message'])) ?></div>
                                                </div>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php endif; ?>
                                ...
                                <?php if ($ticket['status'] === 'Resolvido'): ?>
                                    <div class="alert alert-secondary mb-0">
                                        <i class="fas fa-lock me-2"></i> Este chamado foi resolvido e não aceita novas respostas do usuário.
                                    </div>
                                <?php else: ?>
                                    <form method="post" action="index.php" class="mt-3">
                                        <input type="hidden" name="ticket_id" value="<?= (int)$ticket['id'] ?>">
                                        <input type="hidden" name="user_reply_ticket" value="1">
                                        <label class="d-block mb-2 fw-semibold text-arruda-deep">
                                            <i class="fas fa-reply me-1"></i> Responder ao chamado
                                        </label>
                                        <textarea name="reply_message" rows="4" class="form-control" placeholder="Escreva sua resposta para o suporte..." required></textarea>
                                        <div class="d-flex justify-content-end mt-3">
                                            <button type="submit" class="btn btn-arruda-primary btn-sm" data-loading-text="Enviando resposta...">
...
<<<<<<< HEAD
        document.querySelectorAll('form').forEach(function(form) {
            form.addEventListener('submit', function(e) {
                ...
                var loadingText = btn.dataset.loadingText || 'Enviando...';

                if (form.querySelector('input[name="user_reply_ticket"]')) {
                    loadingText = 'Enviando resposta...';
                } else if (form.querySelector('input[name="delete_ticket"], input[name="delete_user"], input[name="delete_admin"]')) {
                    loadingText = 'Excluindo...';
                } else if (form.querySelector('input[name="update_ticket"], input[name="create_admin"], input[name="create_user"], input[name="update_admin"], input[name="update_user"], input[name="reply_message"]')) {
        ...
        // Esse código fofo faz o navegador "esquecer" os dados do formulário
        // depois que a página carrega, impedindo o reenvio ao atualizar (F5)
